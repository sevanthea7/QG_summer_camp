...
class ImdbDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        with open(file_path, encoding='utf-8') as f:
            token = tokenlize(f.read())

            ws = np.load("../data/aclImdb_trans.npy", allow_pickle=True).item()
            # ws = pickle.load(open("./model/aclImdb_ws.pkl", "rb"))    # 加载 ws 表
            token = temp_transform.trans(sentence=token, ws=ws, max_len=LSTM_hyper_parameters.max_len)
            return np.array(token), np.array(label)
...
def get_dataloader(train=True):
    temp = ImdbDataset(LSTM_hyper_parameters.train_data_path, LSTM_hyper_parameters.test_data_path, train=train)
    data_loader = DataLoader(temp, batch_size=LSTM_hyper_parameters.train_batch_size,
                             shuffle=LSTM_hyper_parameters.train_shuffer)
    return data_loader
